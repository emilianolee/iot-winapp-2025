...
- C# : **마이크로소프트**가 개발한 객체지향 프로그래밍 언어
    - 1990년 귀도 반 로섬이 Python을 개발(1989년 개발 시작)
    - 1985년 비야네 스트롭스트룹이 C++을 개발(너무 어려움, 1980년 개발 시작)
    - 1995년 제임스 고슬링이 Java를 개발(C++의 문법을 기반, 좀 더 쉬운 언어, 1991년 개발 시작)
    - MS가 썬 마이크로시스템즈와 라이센스 체결 후 MS용 Java인 J++을 개발. 특허권 소송 발생
...
- C# 특징
    ...
    - 문법이 Java와 상당히 유사. C/C++ 보다 난이도가 낮음
...
- C# 버전
    - C# 1.x - 2002 ~ 2003년. .NET Framework 1.x문제 많음
...
- Windows Forms(문법 학습): 가장 오랜된 윈앱 개발
...
4. PyQt Designer와 거의 유사
    ...
    - MaximizeBox > False
    - FormNorderStyle > Sizable -> FixedSingle로 변경

5. 보기 > 도구 상자 클릭
    - 공용 컨트롤 > Button 드래그
    - 더블클릭, 클릭으로 선택 후 폼에서 왼쪽 버튼 누르고 드래그 등 다양한 방법 有
...
8. 중간마다 Ctrl + S(저장), Crtl + Shift + S(모두 저장) 눌러서 저장 습관
...
11. 오류 발생 시
    - 폼 디자인의 컨트롤 이벤트와 로직 코드 상의 이벤트 처리 메서드 생성사 불일치로 발생
    - 이벤트 이름은 생성되고, 이벤트 저의 메서드는 생성이 안되서 오류 발생
...
- `TextBox` : 텍스트 입력을 위한 컨트롤
    ...
    - PasswordChar : 들어가는 텍스트를 암호처럼 숨길 때 사용. 보통 특수문자 '●' 를 사용 
    - Multiline : 여러줄 사용여부. true가 되어야 높이 조절 가능
    - PlaceholderText : 입력 전 입력내용 표시
    - ReadOnlt : 입력 가능 여부, True는 입력 불가
...
- `ComboBox` : 여러개 중 아이템을 선택하는 컨트롤
    ...
    - **SelectedIndexChanged** : 선택한 아티엠 순번이 바뀔 때 발생하는 이벤트
    - **SelectedValueChange** : 선택한 아이템 값이 바뀌 때 발생하는 이벤트 
...
- 각 IDE별 한글 깨짐 해결 방벙
    - VS Code > 오른쪽 하단 상표시줄 UTF-8 클릭 인코딩하여 다시 열기 또는 인코딩하여 저장 실행
...
- 기본 문법
    1. 문자열 처리 - [소스](./day02/Day02Study/SyntaxWinApp04/FrmMain.cs)
        ...
        <img src="./image/cs004.png" width=600>
    ...
    4. 상속, 다형성, 추상클래스, 인터페이스 - [소스](./day03/SyntaxWinApp02/SyntaxWinApp02/Computer.cs)
        ...
        <img src="./image/cs005.png" width=600>
...
- `NumericUpDown` : 숫자 입력을 도와주는 컨트롤. 
    ...
    - Maximum : 입력할 수 있는 최대값
    - Value : 현재값
    - 이벤트 생성 거의 안함
    - 나머지는 Button과 동일

- `RichTextBox` : 포맷팅이 가능한 텍스트박스
    - MS Word, 한글워드프로세스 같은 문장 꾸미기, 페이지 꾸미기가 가능
    - Rtb~ 로 시작
    - ScrollBars : 컨트롤 오른쪽, 하단에 스크롤바 생성
        - Both 보다는 ForcedBoth, ForcedVertical 사용
    - WordWrap : 컨트롤 길이를 넘어가면 자동으로 아래로 줄바꿈 여부
    - Font : 대표적인 폰트 사용할 것
...
- 보기 > 개체 브라우저
- 소스코드가 회색으로 연하게 변하는 것 > 되도록이면 사용을 피라하는 뜻 > Alt+Enter로 변경권장사항 확인
...
- 기본 문법
    ...
    2. 예외처리
        - 실행 중 비상종료를 막지 위해서 사용
        - try ~ catch ~ finally 구문으로 처리
        - 모든 예외의 부모클래스는 Exception 클래스
        - 예외클래스를 모르겠다 싶으면 Excepton으로 처리

        <img src="./image/cs007.png" width=600>

        https://github.com/user-attachments/assets/6623542f-f6bc-4f4c-98d0-142bec7578fc

    3. 컬렉션 - [소스](./day04/Day04Study/WinFormsApp002/FrmMain.cs)
        - 배열, 리스트, 딕셔너리, 큐, 스택 등 여러 데이터를 저장하고 다루는 구조를 가진 객체를 통칭
        - 배열로 콤보박스에 데이터 할당
        - 리스트로 콤보박스 데이터 할당
        - 딕셔너리로 콤보박스 데이터 할당

        <img src="./image/cs009.png" width=600>
    
    4. 파일입출력 - [소스](./day04/Day04Study/SyntaxWinApp003/FrmMain.cs)
        - 파일 쓰기, 읽기 가능
        - SaveFileDialog, OpenFileDialog, ColorDialog 컨트롤 사용

        <img src="./image/cs010.png" width=600>
...
- `StatusStrip` : 상태표시줄 컨트롤. 서브 컨트롤 추가 가능
    - 이름은 거의 변경 안함. 대신 서브 컨트롤 이름을 변경
    - `StatusLabel` : 글자만 표시, Lbl~ 로 이름 시작
    - `ProgressBar` : 진행바 표시, Prg~ 로 이름 시작
...
- 기본 문법
    1. 델리게이트(대리자), 이벤트 - [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
        ...
        - 대리자는 실행할 메서드를 가지고 있고, 대리자를 호출하면 대리자가 메서드를 대신 호출
        - 대리자는 호출할 메서드와 파라미터가 일치해야 함
        - 윈폼에서 버튼을 클릭하면, 직접 메서드를 호출하는게 아니고, 
        - C#이 사용자의 움직임을 체크하고 있다가 클릭`이벤트가 발생하면 그에 해당하는 메서드를 대신 실행(호출)시켜주는 것`
        ...
        - **이벤트** : 대리자의 특별한 형태. 어떤일이 발생하면 연결된 메서드를 호출하는 것, PyQT의 `시그널`과 동일
            - 윕앱 버튼클릭, 마우스 드래그, 타이머종료
    ...
    <img src="./image/cs011.png" width=600>
...
- 고급 문법
    ...
    3. 속성 - Property
        - 객체지향 클래스에서 멤버변수(명사) 중에서 public 변수
        - 첫번째 글자 대문자
        - { get; set; } 형태로 구성
    ...
    5. 제네릭
        ...
        ```
        - 위 3개의 메서드를 아래의 하나의 매서드로 퉁칠 수 있음
        ...
        <img src="./image/cs012.png" width=600>

    6. 비동기, 스레드 - [소스](./day05/Day05Study/SyntaxWinApp03/FrmMain.cs)
        - UI 프로그램에 `응답없음` 발생
        - 멀티프로세스 - 한꺼번에 여러개의 프로세스를 실행
        - 스레드 : 프로세스에서 여러일을 한꺼번에 수행하기 위해 분리한 업무 단위
        - 멀티스레드 : 한 프로세스에서 여러 스레드 동작하는 것
        - 응답없음이 발생하면 프로그램의 신뢰도가 떨어짐
        - 윈앱 - 싱글스레드(UI가 스레드를 제어)
        - 콘솔에는 실시간 출력, 텍스트 박스, 프로그레스바 등 컨트롤 표시가 실시간으로 안됨
        ...
        <img src="./image/cs013.png" width=600>
...
### 윈앱 컨트롤 5
- `BackgroundWorker` : 화면 뒷단에서 작업할 일 중 스레드 처리가 되어야할 부분을 손쉽게 동작시켜주는 컨트롤(화면 표시 X)
    - Bgw~ 이름 사용
    - 일반 속성은 거의 사용 안함
    - 이벤트 3가지는 모두 사용
        - `DoWork` : 백그라운드(스레드) 작업
...
- `CheckBox` : 여러개에서 다수를 선택할 수 있는 컨트롤(라디오버튼과 동작이 반대)
    ...
    - CheckState :
        - Unchecked : 체크를 안한 상태
        - Checked : 체크한 상태
        - Intermediate : 모호한 상태(하위 체크가 일부는 체크되고 일부는 체크 안됐을 때)
...
- `TrackBar` : 사운드, 화면밝기 등 최소, 최대를 스크롤로 조정하는 컨트롤
    ...
    - Scroll : 기본이벤트, 가로로 스크롤 이동 시 발생하는 이벤트
...
- `MemuStrip` : 프로그램 메뉴 관리
    - Mnu~
    - 사용하는 메뉴는 반드시 이름을 변경할 것 (예: 끝내기XMenustrip)와 같이 이름이 생성
    - 메뉴 이름 작성시 (&X)와 같이 사용하면 영문자 아래 밑줄 표시됨
    - Click : 해당메뉴 클릭 이벤트

- `ToolTip` : 각 컨트롤에 툴팁아이콘 표시 컨트롤
    - 툴팁컨트롤 하나로 모든 컨트롤에 툴팁 관리 가능
    - Form_Load에서 작성
...
### C# 마지막
- C# 고급
    1. 비동기, 스레드 - [소스](./day06/Day06Study/SyntaxWinApp06/FrmMain.cs)
        - `네번째` 해결방법
            - BackgroundWorker 클래스 사용
            - .NET에 특화된 스레드사용 클래스
            - 전통적인 스레드를 쉽게 쓸 수 있게 변형
            - this.invoke, async, await, Task 등 사용할 필요 없음
            - 취소 기능을 추가할 수 있음
...
- 윕앱 응용 개발
    ...
    <img src="./image/cs014.png" width=800>
...
### C# 응용 - WPF
- WPF 개요
    - `Windows Presentation Foundation` 약자. 윈도우에서 무언가를 표시하기 위한 기초
    - 윈앱(윈폼즈)을 개선한 버전. 윈앱에서는 이미지 표현이 비트맵방식(2D). 따라서, 용량문제, 계단 현상 문제가 존재
    - WPF에서는 비트맵방식의 문제를 해결 -> 벡터방식(텍스트). 속도개선, 용량절약

- WPF 특징
    - WPF UI 디자인에는 이미지 `벡터`방식 사용. 비트맵 방식도 혼용가능 
    - `XAML` : XML기반의 디자인 문법으로 화면 UI를 디자인함. PyQt와 동일 
        - 안드로이드 등 모바일 개발, 자바 FX, Qt디자인 등 화면 UI의 최신 트렌드
    - HTML로 디자인하던 디자이너가 손쉽게 적굥할 수 있음
    - 백엔드는 C# 윈앱과 거의 동일, 프론트엔드만 차이
        - 윈앱과는 다르게 코딩으로 디자인을 수행. 윈앱처럼 디자이너 사용 가능
        - 디자이너 -> Blend for Visual Studio 디자인 툴 사용
        - 개발자 -> VS에서 코딩으로 디자인
    
    <img src="./image/cs017.png" width=400>

- XAML
    - `eXtensible Application Markup Language` : XML 기반 언어
    - WPF(Windows), UWP(Win, Linux, MacOS), MAUI(모바일) 동일하게 사용
    - 태그형태로 디자인. <열린태그></닫힌태그>, <태그 내 다른 태그 없을 때 />
    ```xml
...
- WPF 프로젝트 구조
    ...
    - App.xaml : 앱 실행을 관리하는 리소스 파일
        ...
        - 윈앱에 Program.cs와 동일
    - MainWindow.xaml : Form1.Designer.cs 와 동일한 디자인 파일
...
- WPF 실제개발법(XAML) - [소스](./day07/Day07Study/WpfStudyApp02/MainWindow.xaml)
    ...
    - **Grid** 컨트롤 : WPF에서 가장 많이 사용하는 레이아웃(중요!)
        ...
        - `Grid.Row`, `Gird.Column` : 그리드 상 컨트롤 위치할 행과 열 인덱스
        - Margin : 외부 여백
            - 10 -> 상하좌우 다 10의 공백을 여백
            - 10, 20 -> 좌우 여백, 상하 여백
            - 10, 5, 20, 30 -> 좌, 상, 우, 하 여백
        - Padding: 컨트롤 내부 여백
        - 디자인할 창의 구역을 나눌때 사용

        <img src="./image/cs015.png" width=600>
...
- 데이터바인딩 - [디자인](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml) [소스](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml.cs)
    ...
    - 바인딩 방식
        1. 직접코드 할당 방식
            ...
            - DataContext와 관계 없음
            - 단순한 속성 사용방식
        2. XAML Binding 방식 : MVVM 디자인패턴 시 필수
            - {Binding fruitItems}
            - DataContext 기반으로 속성명 fruitItems를 검색
            - DataContext에 없으면 실패
            - BindingManager가 작업을 수행
            - DataContext는 컨트롤 자체에 할당해도 됨
    - ItmesSource : 각 컨트롤이 데이터를 바인딩하는 속성
    - DataContext : XAML이 데이터를 어디에서 가져오는지 설정하는 속성
        - 데이터 한건만 설정X
        - 클래스 객체를 패키지로 전달 가능 
    - 바인딩 순서
        1. WPF 바인딩 엔진은 DataContext 객체 안에서 public 접근제어자의 fruitsItems라는 값을 검색
    ...
    - 바인딩 모드 : 총 4가지 모드. 그 중 2가지만 알면 됨
        ...
        2. `OneWay` : 원본 데이터 -> 사용중인 데이터도 변경
        3. OneWayToSource : 사용중인 데이터 변경 -> 원본을 변경
...
- 데이터베이스 데이터 바인딩
    ...
    <img src="./image/cs016.png" width=600>
...
- 네비게이션 - [소스](./day08/Day08Study/WpfStudyApp02/MainWindow.xaml)
    ...
    - NavigationUIVisibilty 속성

- 비트뱁 디자인, 벡터 디자인 - [소스](./day08/Day08Study/WpfStudyApp03/MainWindow.xaml)
...
- 컨트롤 디자인,  리소스 - [소스](./day08/Day08Study/WpfStudyApp04/MainWindow.xaml)
    ...
    <img src="./image/cs019.png" width=600>
...
- 사용법 - [소스](./day08/Day08Study/WpfStudyApp05/MainWindow.xaml)
    ...
    <img src="./image/cs020.png" width=600>
...
- 연습예제
    ...
    <img src="./image/cs021.png" width=600>
...
### WPF UI 프레임워크
- 개요
    - Fluent UI라는 이름에 Modern UI의 한 스타일 UI 프레임워크
...
- 기본 사용법 - [소스](./day09/Day09Study/WpfStudyApp03/MainWindow.xaml)
    ...
    - MainWindow.xaml에 xmlns:ui 추가. App.xaml과 동일
        ```xml
        xmlns:ui="http://schemas.lepo.co/wpfui/2022/xaml"
        ```
    
    - MainWindow.xaml에 Window -> ui.FluentWindow로 변경
    - MainWindow.xaml.cs에 public partial class MainWindow : Window -> public partial class MainWindow : FluentWindow로 변경
    - Grid 내 타이틀 바 추가
        ```xml
        <ui:TitleBar Title="WPF UI App"/>
        ```
    - Theme를 Dark로 했을 경우
        - Window 전체 Background를 어두운색으로 지정 후 작성

    - 실행결과 

        <img src="./image/cs022.png" width=600>

        Dark Theme

        <img src="./image/cs023.png" width=600>
